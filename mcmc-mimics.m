...
f_met1=load ('f_met.mat');%  Unit: g C/ m2/yr for litter input
    ...
    meanInputToLitter1=f_met1.meanInputToLitter1;
        meanInputToLitter1(meanInputToLitter1<=0)=NaN; 
    meanInputToLitter2And3=f_met1.meanInputToLitter2And3;
        meanInputToLitter2And3(meanInputToLitter2And3<=0)=NaN;
...
KO_r=4;KO_K=4;
% NOTE: for microbial turnover, I directly use turnover time instead of
% using the follwoing two equation
% Tau_mod: modify microbial turnover
Tau_mod_K=1; 
Tau_mod_r=1; 
...
% The flux among pools: start
% Uptake_LITm_MICr=MICr*Vmax_r1*LITm/(Km_r1+LITm); % eq 1. Not used!
% Uptake_LITs_MICr=MICr*Vmax_r2*LITs/(Km_r2+LITs); % eq 2 Not used!
%Uptake_SOMa_MICr=MICr*Vmax_r3*SOMa/(Km_r3+SOMa); % eq 3
Decay_MICr_SOM=MICr*Tau_r;                       % eq 4
% Uptake_LITm_MICk=MICk*Vmax_K1*LITm/(Km_K1+LITm); % eq 5 Not used!
% Uptake_LITs_MICk=MICk*Vmax_K2*LITs/(Km_K2+LITs); % eq 6 Not used!
%Uptake_SOMa_MICk=MICk*Vmax_K3*SOMa/(Km_K3+SOMa); % eq 7
Decay_MICk_SOM=MICk*Tau_K;                       % eq 8
    
    %% calculating steady states
I_LITm=meanInputToLitter1;I_LITs=meanInputToLitter2And3;
SOMp=(I_LITm*f_i_met+f_p_r.*Decay_MICr_SOM+f_p_K.*Decay_MICk_SOM)./D;
Decay_SOMp_SOMa=SOMp.*D;                          % eq 9

% solve second order equation to derive SS_SOMc & SS_SOMa
    % define all the coefficients for SS_SOMc
    InputToSOMc=I_LITs*f_i_struc+f_c_r.*Decay_MICr_SOM+f_c_K.*Decay_MICk_SOM; %gC/m2/yr
